...
const MAP_STYLE = {
  ...
  layers: [
    { id: "background", type: "background", paint: { "background-color": "#071827" } },
...
<body>
  ...
  <script>
    ...
    const prefetchedTileUrls=new Set(),tilePrefetchQueue=[];let activeTilePrefetches=0,prefetchTimer=0,glowTimer=0,glowFrame=0;
    ...
    function scheduleNodeGlow(){clearTimeout(glowTimer);cancelAnimationFrame(glowFrame);glowTimer=setTimeout(()=>{if(!map.getLayer("clip-points")||!map.hasImage("top-star"))return;const startedAt=performance.now(),normal=[57,217,204],highlight=[143,245,235],render=now=>{const progress=Math.min(1,(now-startedAt)/440),strength=Math.sin(progress*Math.PI),color=normal.map((channel,index)=>Math.round(channel+(highlight[index]-channel)*strength));map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"rgb("+color.join(",")+")","#7c9299"]);const glowingStar=makeTopStar(strength);if(glowingStar)map.updateImage("top-star",glowingStar);if(progress<1)glowFrame=requestAnimationFrame(render);else{map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"#39d9cc","#7c9299"]);const baseStar=makeTopStar();if(baseStar)map.updateImage("top-star",baseStar)}};glowFrame=requestAnimationFrame(render)},60)}
    ...
    let mapReady=false;
    const initializeMapLayers=()=>{if(!map.isStyleLoaded()||map.getSource("clips"))return;
      addTopStar();
      ...
      map.addSource("clips",{type:"geojson",data:placesToGeoJson(places),cluster:false});
      ...
      map.addSource("active-cluster",{type:"geojson",data:{type:"FeatureCollection",features:[]}});
       ...
       map.addLayer({id:"clip-points",type:"circle",source:"clips",filter:["all",["!",["has","point_count"]],["==",["get","top"],false]],paint:{"circle-radius":["case",["get","linked"],5.5,4.5],"circle-color":["case",["get","linked"],"#39d9cc","#7c9299"],"circle-color-transition":{duration:0,delay:0},"circle-stroke-color":"#071827","circle-stroke-width":1.5}});
       ...
       map.addLayer({id:"active-clip-point",type:"circle",source:"active-clip",filter:["==",["get","top"],false],paint:{"circle-radius":["case",["get","linked"],10.5,8.25],"circle-color":["case",["get","linked"],"#55eadc","#91a5ac"],"circle-stroke-color":"#b9fff7","circle-stroke-width":2,"circle-blur":.06}});
       map.addLayer({id:"active-top-halo",type:"circle",source:"active-clip",filter:["==",["get","top"],true],paint:{"circle-radius":14,"circle-color":"rgba(0, 0, 0, 0)","circle-stroke-color":"#65eadf","circle-stroke-width":2.1,"circle-stroke-opacity":.8}},"top-points");
    ...
    };map.on("styledata",initializeMapLayers);map.on("load",initializeMapLayers);map.on("idle",initializeMapLayers);initializeMapLayers();
